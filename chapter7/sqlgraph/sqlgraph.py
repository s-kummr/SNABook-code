...
class SqlGraph(object):
    ...
    def remove_node(self, node, cursor=None):
        cursor.execute('delete from "%s_nodes" where node=?' %
                       self.name, (node,))

    @cursored
    def remove_edge(self, fromnode, tonode, cursor=None):
        cursor.execute('delete from "%s_edges" where efrom=? and eto=?' %
                       self.name, (fromnode, tonode))

    @cursored
    def get_node_data(self, node, cursor=None):
        result = cursor.execute(
            'select * from "%s_nodes" where node=?' % self.name, (node,))
        for row in result:
            return row[0]
        else:
    ...
    def get_edge_data(self, fromnode, tonode, cursor=None):
        result = cursor.execute(
            'select * from "%s_edges" where efrom=? and eto=?' % self.name, (fromnode, tonode))
        for row in result:
            return row[0]
        else:
