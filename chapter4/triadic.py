...
__author__ = "\n".join(['Max Tsvetovat (maksim@tsvetovat.org)',
                        'revised from code by Alex Levenson (alex@isnontinvain.com) and Diederik van Liere (diederik.vanliere@rotman.utoronto.ca)'])
...
def triadic_census(G):
    ...
    node_census = dict((v, dict((name, 0) for name in triad_names))
                       for v in G.nodes())
    n = len(G)
