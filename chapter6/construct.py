    ...
    def _roulette_choice(self, names, values, inverse=False):
        ...
        for i in range(len(names)):
            if not inverse:
                ...
                wheel.extend([names[i]
                              for x in range(1+int((1-values[i])*10))])
...
for x in g.nodes():
    for y in g.nodes():
        if r.random() <= density:
            g.add_edge(x, y, weight=r.random())
...
for i in range(runtime):
    ...
    for f, t, data in g.edges(data=True):
        ...
        if data['weight'] < 0.1:
            dropped_edges.append((f, t))
    for (f, t) in dropped_edges:
        g.remove_edge(f, t)
...
# ,edge_color=ew,edge_cmap=plot.cm.RdPu)
net.draw_networkx(g2, node_color=col, cmap=plot.cm.Reds)
